...
describe("createMemo", () => {
  ...
  it("メモを保存し、保存時刻を記録する", async () => {
    ...
    const stored = await listMemos(db);
  ...
  it("前後の空白を除去して保存する", async () => {
    const db = createTestDb();
    await createMemo(db, { content: "  余白あり  ", now: 1 });
  ...
  it("検証に失敗したメモは保存しない", async () => {
    ...
    const result = await createMemo(db, { content: "   ", now: 1 });

    expect(result).toEqual({ ok: false, error: "empty" });
    await expect(listMemos(db)).resolves.toEqual([]);
  });
  ...
  it("メモごとに異なる識別子を割り当てる", async () => {
    const db = createTestDb();
    await createMemo(db, { content: "A", now: 1 });
    await createMemo(db, { content: "B", now: 2 });
...
describe("listMemos", () => {
  ...
  it("保存時刻の新しい順に返す", async () => {
    const db = createTestDb();
    await createMemo(db, { content: "古い", now: 1_000 });
    await createMemo(db, { content: "最新", now: 3_000 });
    await createMemo(db, { content: "中間", now: 2_000 });
  ...
  it("保存直後のメモが先頭に含まれる", async () => {
    const db = createTestDb();
    await createMemo(db, { content: "既存", now: 1_000 });

    await createMemo(db, { content: "たった今", now: 2_000 });
  ...
  it("他の利用者のメモを含まない", async () => {
    const db = createTestDb();
    await createMemo(db, { content: "自分のメモ", now: 1_000 });
    await createMemo(db, {
      content: "他人のメモ",
      now: 2_000,
      userId: "another-user",
    ...
    const stored = await listMemos(db, SINGLE_USER_ID);
