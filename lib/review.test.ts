...
import { createMemo, deleteMemo, SINGLE_USER_ID } from "./memos";
import {
  createQuizItem,
  validateQuizItem,
  getReviewStates,
...
async function seedMemo(db: AppDb, content: string, now = NOW) {
  const r = await createMemo(db, { content, now });
...
describe("createQuizItem（タスク 3.1）", () => {
  it("作成すると初回の出題日が1日後になる", async () => {
    ...
    const r = await createQuizItem(db, {
      memoId: memo.id, question: "定休日は？", answer: "火曜日", now: NOW,
    });
  ...
  it("片方だけでは保存しない", async () => {
    ...
    const r = await createQuizItem(db, {
      memoId: memo.id, question: "問だけ", answer: "", now: NOW,
    });
  ...
  it("1メモに2つ目の問答は作れない", async () => {
    const db = createTestDb();
    const memo = await seedMemo(db, "本文");
    await createQuizItem(db, { memoId: memo.id, question: "問", answer: "答", now: NOW });

    const second = await createQuizItem(db, {
      memoId: memo.id, question: "別の問", answer: "別の答", now: NOW,
    });
  ...
  it("他人のメモには作れない", async () => {
    const db = createTestDb();
    const r = await createMemo(db, { content: "他人のメモ", now: NOW, userId: "someone-else" });
    if (!r.ok) throw new Error("失敗");

    const created = await createQuizItem(db, {
      memoId: r.memo.id, question: "問", answer: "答", now: NOW,
    });
...
describe("一覧の復習状態（タスク 3.2）", () => {
  it("問答の有無で2つの状態を返す", async () => {
    ...
    await createQuizItem(db, { memoId: withQuiz.id, question: "問", answer: "答", now: NOW });

    const states = await getReviewStates(db);
  ...
  it("未作成の件数を数えられる", async () => {
    ...
    await seedMemo(db, "B");
    await seedMemo(db, "C");
    await createQuizItem(db, { memoId: a.id, question: "問", answer: "答", now: NOW });

    await expect(countUnwritten(db)).resolves.toBe(2);
  });
});
...
describe("出題対象（タスク 4.1）", () => {
  async function seedThree(db: AppDb) {
    ...
    await createQuizItem(db, { memoId: due.id, question: "来てる？", answer: "はい", now: NOW });
    await createQuizItem(db, { memoId: future.id, question: "まだ？", answer: "はい", now: NOW });
  ...
  it("期日が来たものだけが対象になる", async () => {
    ...
    // 翌日になれば due は期日を迎える
    const items = await getDueItems(db, later(1));
  ...
  it("期日前は対象にならない", async () => {
    ...
    await expect(getDueItems(db, NOW)).resolves.toEqual([]);
  });
  ...
  it("他人のメモは対象にならない", async () => {
    ...
    await createQuizItem(db, { memoId: mine.id, question: "問", answer: "答", now: NOW });

    await expect(getDueItems(db, later(1), "someone-else")).resolves.toEqual([]);
  });
});
...
describe("自己採点の記録（タスク 4.2）", () => {
  async function seedDue(db: AppDb) {
    ...
    const [item] = await getDueItems(db, later(1));
    return item;
  }
  ...
  it("覚えてたを記録すると3日後へ進む", async () => {
    ...
    const r = await gradeReview(db, {
      ...
      occurrenceAt: item.occurrenceAt, now: later(1),
    });
  ...
  it("採点した当日には再び出題されない", async () => {
    ...
    await expect(getDueItems(db, later(1))).resolves.toEqual([]);
  });
});
...
describe("記録のべき等性（タスク 4.3・design.md D4）", () => {
  ...
  it("存在しない問答は not_found を返す", async () => {
    const db = createTestDb();
    const r = await gradeReview(db, {
      quizItemId: "missing", recalled: true, occurrenceAt: 0, now: NOW,
    });
  ...
  it("他人の問答は採点できない", async () => {
    ...
    const r = await gradeReview(db, {
      quizItemId: item.quizItemId, recalled: true,
      occurrenceAt: item.occurrenceAt, now: later(1), userId: "someone-else",
    });
...
describe("メモの削除", () => {
  it("削除すると出題対象からも外れる", async () => {
    ...
    await createQuizItem(db, { memoId: memo.id, question: "問", answer: "答", now: NOW });
    expect(await getDueItems(db, later(1))).toHaveLength(1);

    const r = await deleteMemo(db, { memoId: memo.id });
  ...
  it("他人のメモは削除できない", async () => {
    const db = createTestDb();
    const r = await createMemo(db, { content: "他人の", now: NOW, userId: "someone-else" });
    if (!r.ok) throw new Error("失敗");

    const deleted = await deleteMemo(db, { memoId: r.memo.id });
