...
- [x] 9.2 わざと型エラーを入れたブランチを push し、**CI が赤くなることを実際に確認する**。確認後に戻す
  - `harness/ci-red-probe` に型エラーを 1 つ入れて push し、CI が
    `ci-red-probe.ts(1,14): error TS2322` ちょうどで赤くなることを確認した。
    確認後にリモート・ローカルとも削除済み
  - **門は fail closed なので、このコミットは作れなかった。** `.claude/settings.json` を
    一時的に外して作った。迂回できないこと自体は設計どおりなので、門は緩めていない
  - その手前で、CI は**仕掛けたものではない欠陥を 4 件**出した。
    (1) tsc が Linux で起動しない (2) vitest（rolldown）が Linux で起動しない
    (3) cron worker の依存を CI が入れていない (4) biome が診断を 20 件で打ち切り、
    注入したファイルが恒常的な警告の陰に隠れる。
    (1)〜(3) は手元では出ない。(4) は出る順しだいなので手元でも起こりえたが、
    実際に出たのは CI である。修正は 5 コミットあるが、そのうち 1 つ
    （`npm ci` を `npm install` に替えたもの）は**私の直し方が間違っていた**もので、
    新しい欠陥ではない
...
- [x] 10.3 `npm run check` を最初から通し、全部緑であることを確認する
- [x] 10.4 `bash scripts/scan-secrets.sh` を**コミット 2・3 を作った後に**走らせる（履歴の blob が入力なので、コミット前では新しい行が入力に入らない — L09）
- [x] 10.5 このリポジトリで実際に 1 コミットを最後まで通し、3 段の hook が順に働くことを確認する（編集で整形 → ターン終了で test → コミットで検査とレビュー）
  - 整形: 未整形の `.ts` を Write して、保存後に整形されていることを確認（`.md` では発火しないことも）
  - 終了: わざと型エラーを残してターンを終えようとして、実際にブロックされた
  - コミット: 受領書が無い状態で止められ、`harness:review` の指摘 8 件を直してから通った
