...
**実装時に spec とぶつかった。** `first-run` の「答えたことが残る」は
「移って戻ったとき、同じ問いを示さず**答えたあとの状態のままにする**」と
書いていた。上の形だと告知ごと消えるので、後半を満たさない。

差し出しが画面の中の状態として持ち上げられていた頃の書き方である。画面が
それぞれ固有の経路を持つと移って戻るたびに描き直されるので、礼を残すには
答えを端末側に保存し続けることになる。**一度きりの礼のために状態を持ち越す
価値は無い**と判断し、spec を改定した（利用者の判断）。残すべきなのは
「もう問わない」であって、「答えた」という表示ではない。
...
### D13: 絞り込みは経路に持ちつつ、タブは端末側にも憶える

**実機で発覚した。** 絞り込んだ一覧から詳細を開き、**下部タブの「メモ」で
戻ると絞り込みが外れる**。PWA にはブラウザの戻るが無く、下部タブが最も自然な
戻り道になる。`memo-capture` の「戻ったときに絞り込みの状態を保つ」を
満たしていなかった。

2 段で直した。

1. 詳細の経路に絞り込みを持って行く（`/memos/<id>?tag=…`）。詳細にいる間も
   タブのリンクが `?tag=` を組み立てられる
2. **それだけでは足りない**（レビューの指摘）。`/review` と `/record` は
   絞り込みを持たないので、復習へ移って戻ると消える。メモ側の経路にいる間に
   `sessionStorage` へ憶え、経路が持たない画面ではそれを使う

**憶えるのはメモ側にいるときだけ。** 復習で `null` を憶えると、戻り先が
素の一覧になる。置き場を `sessionStorage` にしたのは下書きと同じ理由で、
タブを閉じたら忘れてよい寿命だから。

解くのは一覧の「ぜんぶ」の札の仕事で、タブの仕事ではない。解いたときは
経路からも憶えからも消す（`navigation`「絞り込みを解いたときは、経路からも
取り除く」）。
...
変更前（74981d9）と変更後を、それぞれ `next build` して
`.next/static/chunks/*.js` の合計バイト数で比べた。
...
**減っていない。53.4 KB（+6.9%）増えた。**

期待と逆だが、理由は説明がつく。経路が 1 本から 4 本に増え、それぞれに
チャンクが要る（`next/link` と各画面の分）。第12章が言う「Server Components に
すればバンドルが減る」は**同じ画面の中で Client Components を減らしたとき**の
話であって、画面を分けたぶんの増加とは別勘定である。

**この数字は利用者 1 人が読み込む量ではない。** 4 経路ぶんの全チャンクを
足した値で、実際には開いている経路のぶんしか要らない。Turbopack のこの構成では
経路ごとの First Load JS が出力されず、`app-build-manifest.json` も作られない
ため、経路ごとの内訳は取れなかった。

**したがって「増えた／減った」を利用者の体感として断定できない。**
実機（タスク 5.2）でタブ切替の体感を測るときに、併せて見る。
...
- ~~**タブ切替の体感。**~~ **閉じた。** 実機（PWA）で触ってもらい、耐えられると
  の回答を得た。クライアント保持へ戻す退路は使わない
- **焦点の復元が Next.js のスクロール復元だけで足りるか**（D10）。
  **この change では確かめきれなかった。** 実機でしか見られず、利用者が
  あとで見ることになっている。`docs/open-issues.md` の 2 番に移した——
  change を archive するとこの文書は歴史になるので、続きを追える場所へ置く
