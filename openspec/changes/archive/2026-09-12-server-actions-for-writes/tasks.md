...
- [x] 10.1 デプロイして iPhone（PWA）で開いてもらった（Version 7cac1156）。
      **動いているとの回答。** 書き込みの経路で欠陥の報告は無い。
      前の change（5.1）では同じ手順で欠陥が 1 件出ているので、
      この確認は形だけのものではない
- [x] 10.2 戻る操作を含めて**利用者が確認した**（design R2）。
      報告は「うまく動いてた」。`refresh()` は `revalidatePath()` と違って
      Router Cache を捨てないので、R2 で懸念したスクロール位置の喪失は
      **起きる仕組みがそもそも無い**——D5 で `revalidatePath()` を選ばなかった
      ことが、この懸念を実装の時点で消していた。
      **ただし「位置が保たれた」と私が観測したわけではない。** 利用者の
      報告と、捨てる経路が無いという構造上の根拠の 2 つで閉じる
- [x] 10.3 通知の設定のオン・オフを含めて確認してもらった。報告は同上
