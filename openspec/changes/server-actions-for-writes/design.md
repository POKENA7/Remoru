...
- **戻る操作でスクロール位置が保たれるか。** `refresh()` は Router Cache を
  捨てないので保たれる**はず**だが、いまの `router.refresh()` でも同じはずで、
  確かめていない。実機でしか分からない。答えがどちらでも設計は変わらない——
  保たれないなら open-issues に残す
- **クライアントバンドルがどう動くか。** `fetch` の殻とエラー表が減る一方、
  Server Actions の参照が増える。前の change で 773.6 KB → 827.0 KB と
  増えているので、**減ると見込んで報告しない。**測ってから書く
