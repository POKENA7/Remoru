...
- 最初から公開サインアップとする。誰でもアカウントを作れる。 [2026-08-23 変更]
  - 当初は「技術的にマルチユーザー対応で設計しつつ、実際の利用は招待制・限定公開に留め、公開サインアップを開けるのはその後」としていた [Q9]。change 3（認証とデプロイ）の設計時に、招待制を設けない判断に変更した。マルチユーザー対応で設計する点は変わらない。
...
- ~~[assumption] 自由記述のメモから問／答のペアをどう作るか（利用者が手動で作るのか、自動生成するのか）は未確定である。~~ → 2026-08-24 に解消。change 5（add-quiz-generation）で**保存時の自動生成**とした。生成に失敗したメモには手動作成の経路が残る。
- [assumption] 成功指標の合格閾値（記憶定着率が何回目で何％、メモ投入が何秒以内か）は未確定である。 [Q3] [Q10]
- ~~[assumption] 限定公開から公開サインアップへ移行する時期・条件は未確定である。 [Q9]~~ → 2026-08-23 に解消。最初から公開サインアップとしたため、移行という段階自体が無くなった。
- [assumption] 公開サインアップに伴う費用と濫用のリスクをどう抑えるかは未確定である。change 5（AI による問答の生成）が入ると、他人の利用がそのまま課金につながる。 [2026-08-23 追加]
