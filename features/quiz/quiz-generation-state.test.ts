...
async function memoWithPending(db: ReturnType<typeof createTestDb>, pending: number | null) {
  ...
  await db.update(memos).set({ quizPendingSince: pending }).where(eq(memos.id, created.memo.id));
...
describe("一覧の状態", () => {
  ...
  it("生成中のメモは作成中になる", async () => {
    ...
    expect(await getReviewStates(db, "u1", NOW)).toEqual(new Map([[id, { kind: "generating" }]]));
  });
  ...
  it("上限を過ぎた作成中は未作成に落ちる", async () => {
    ...
    expect(await getReviewStates(db, "u1", NOW)).toEqual(new Map([[id, { kind: "unwritten" }]]));
  });
